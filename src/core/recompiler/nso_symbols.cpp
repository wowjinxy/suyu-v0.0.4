...
NsoDynamicSymbolParseResult ParseNsoDynamicSymbols(const DecodedNso& image,
    ...
    if (symbol_count > max_symbols || symbol_count > std::numeric_limits<std::size_t>::max()) {
    ...
    NsoDynamicSymbolInfo info{
        .symbol_table_address = expected_symbol_address,
        .symbol_table_byte_size = image.info.dynsym.size,
        .string_table_address = expected_string_address,
        .string_table_byte_size = image.info.dynstr.size,
    };
    ...
    try {
        ...
        for (std::uint64_t index = 0; index < symbol_count; ++index) {
            ...
            NsoDynamicSymbol symbol{
                ...
                .address = expected_symbol_address + offset,
            ...
            if (symbol.name_offset >= string_bytes.size()) {
                result.error = "dynamic symbol " + std::to_string(index) +
                               " has a name offset outside the NSO dynstr extent";
                return result;
            }
            ...
            if (name_end == string_bytes.end()) {
                result.error = "dynamic symbol " + std::to_string(index) +
                               " has no null terminator inside the NSO dynstr extent";
                return result;
            }
